# Pilotys cleaner correspondence + owner note v4
...
```bash
bash pilotys-cleaner-correspondence-owner-note-v1/scripts/install-cleaner-correspondence-owner-note-v4.sh
cd apps/cleaner-web
npm run build
```
...
Patches the main cleaner mission page using the actual variable names from the current page:

- loaded mission row: `mission`
- route token: `token`

It adds `owner_note`, `cleaner_priority_note`, and `owner_note_updated_at` to the mission query, then places the priority note at the top of the mission card. It also adds a `Briefing séjour` link to `/mission/[token]/reservation`.
