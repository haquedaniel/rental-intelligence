...
export async function saveAndRecalculateAdminPricing(f:FormData){
 ...
 const payload={property_id:pid,enabled:f.get("enabled")==="on",mode:text(f,"mode")==="apply"?"apply":"shadow",publication_paused:f.get("publication_paused")==="on",default_price:num(f,"default_price",100),default_weekend_price:optional(f,"default_weekend_price"),floor_price:num(f,"floor_price",50),ceiling_price:optional(f,"ceiling_price"),default_min_stay:Math.max(1,Math.trunc(num(f,"default_min_stay",2))),one_night_gap_multiplier:Math.max(1,num(f,"one_night_gap_multiplier",1.5)),one_night_release_days:Math.max(0,Math.trunc(num(f,"one_night_release_days",21))),protect_weekends:f.get("protect_weekends")==="on",planning_horizon_days:Math.max(30,Math.trunc(num(f,"planning_horizon_days",540))),publication_price_increment:Math.max(.01,num(f,"publication_price_increment",1)),publication_min_change_eur:Math.max(0,num(f,"publication_min_change_eur",1)),optimisation_curve:json(f,"optimisation_curve"),optimisation_preset:text(f,"optimisation_preset")||"progressive",optimisation_horizon_days:Math.max(7,Math.trunc(num(f,"optimisation_horizon_days",120))),optimisation_max_discount_pct:Math.max(0,Math.min(80,num(f,"optimisation_max_discount_pct",30))),market_signal_enabled:f.get("market_signal_enabled")==="on",market_signal_influence_pct:Math.max(0,Math.min(100,num(f,"market_signal_influence_pct",0))),updated_at:new Date().toISOString()};
