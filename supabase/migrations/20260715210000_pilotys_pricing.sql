...
create unique index if not exists pricing_publication_actions_open_unique on public.pricing_publication_actions(property_id,date,action_type) where status in ('proposed','applying');
...
grant all on table public.pricing_property_settings, public.pricing_seasons, public.pricing_date_overrides, public.pricing_daily_prices, public.pricing_publication_actions to service_role;
