...
function PilotysMark({ compact = false }: { compact?: boolean }) {
  return (
    <div className="flex items-center gap-2.5">
      <span className="relative flex h-9 w-9 items-center justify-center rounded-2xl bg-[#112532] text-white shadow-sm">
        <span className="absolute -right-1 -top-1 h-4 w-4 rounded-full bg-[#E0680E]" />
        <svg viewBox="0 0 36 36" className="relative h-6 w-6" aria-hidden="true">
          <path d="M9 28V8h12.5c5.2 0 8.5 3 8.5 7.4 0 4.6-3.4 7.7-8.5 7.7H16v4.9H9Z" fill="white" />
          <path d="M16 17.8h5.2c2 0 3.4-.9 3.4-2.4 0-1.6-1.3-2.4-3.4-2.4H16v4.8Z" fill="#80A5B7" />
          <path d="M16 28V18l-7 10h7Z" fill="#E0680E" opacity="0.92" />
        </svg>
      </span>
      {!compact && <span className="text-lg font-black tracking-[0.22em] text-[#112532]">PILOTYS</span>}
    </div>
  );
}
...
function MoneyHero() {
  ...
  return (
    <section className="relative overflow-hidden rounded-[1.9rem] bg-[#112532] p-5 text-white shadow-[0_18px_45px_rgba(17,37,50,0.22)] sm:p-6 lg:p-7">
      <div className="absolute -right-16 -top-20 h-60 w-60 rounded-full bg-[#80A5B7]/28 blur-3xl" />
      <div className="absolute -bottom-24 left-8 h-56 w-56 rounded-full bg-[#E0680E]/22 blur-3xl" />
      ...
      <div className="relative grid gap-5 lg:grid-cols-[0.85fr_1.15fr] lg:items-stretch">
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.18em] text-white/45">Argent qui rentre</p>
              <h2 className="mt-2 text-2xl font-black tracking-tight">CA réalisé live</h2>
            </div>
            <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-black text-white/70 ring-1 ring-white/15">◉ live</span>
          </div>

          <button
            onClick={() => setMetric("realised")}
            className={`w-full rounded-[1.55rem] p-4 text-left ring-1 transition ${metric === "realised" ? "bg-white text-[#112532] ring-white" : "bg-white/8 ring-white/12 hover:bg-white/12"}`}
          >
            <span className="block text-[11px] font-black uppercase tracking-wide opacity-55">mis à jour à 10:42</span>
            <span className="mt-2 block text-5xl font-black tracking-tight sm:text-6xl">{formatMoney(realised, 2)}</span>
            <span className="mt-3 inline-flex rounded-full bg-[#E0680E]/12 px-3 py-1 text-xs font-black text-[#E0680E] ring-1 ring-[#E0680E]/20">
              avance pendant les séjours en cours
            </span>
          </button>

          <div className="grid grid-cols-2 gap-3">
            <MetricButton active={metric === "gross"} onClick={() => setMetric("gross")} label="CA brut annuel" value={formatMoney(gross)} delta="↑ 12 %" />
            <MetricButton active={metric === "net"} onClick={() => setMetric("net")} label="Après variables" value={formatMoney(net)} delta="↑ 9 %" />
          </div>
        </div>
...
function MetricButton({ active, onClick, label, value, delta }: { active: boolean; onClick: () => void; label: string; value: string; delta: string }) {
  return (
    <button onClick={onClick} className={`rounded-[1.35rem] p-4 text-left ring-1 transition ${active ? "bg-white text-[#112532] ring-white" : "bg-white/8 text-white ring-white/12 hover:bg-white/12"}`}>
      <span className="block text-[10px] font-black uppercase tracking-wide opacity-55">{label}</span>
      <span className="mt-2 block text-2xl font-black tracking-tight sm:text-3xl">{value}</span>
      <span className="mt-1 block text-xs font-black text-emerald-300">vs N-1 · {delta}</span>
    </button>
  );
}
...
function MetricBreakdown({ metric }: { metric: MetricId }) {
  if (metric === "realised") {
    ...
    return (
      <div className="rounded-[1.55rem] bg-white/8 p-4 ring-1 ring-white/12">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-sm font-black text-white">Revenus mensuels</p>
            <p className="mt-1 text-xs font-bold text-white/50">Bleu = réalisé. Orange = déjà réservé à venir.</p>
          </div>
          <div className="flex gap-2 text-[10px] font-black text-white/70">
            <span className="inline-flex items-center gap-1"><span className="h-2 w-4 rounded-full bg-[#80A5B7]" />Réalisé</span>
            <span className="inline-flex items-center gap-1"><span className="h-2 w-4 rounded-full bg-[#E0680E]" />À venir</span>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-12 items-end gap-1.5">
          {monthlyRevenue.map((row) => {
            ...
            const totalHeight = Math.max(18, (total / max) * 132);
            const realisedHeight = total ? (row.realised / total) * 100 : 0;
            return (
              <button key={row.month} className="group flex min-w-0 flex-col items-center gap-2">
                <div className={`flex w-full items-end overflow-hidden rounded-t-xl rounded-b-md border ${row.future > 0 ? "border-white/28 bg-[#E0680E]/78" : "border-transparent bg-white/8"} ${row.live ? "shadow-[0_0_18px_rgba(224,104,14,0.58)]" : ""}`} style={{ height: `${totalHeight}px` }}>
                  <div className="w-full rounded-t-lg bg-[#80A5B7]" style={{ height: `${row.realised > 0 ? Math.max(3, realisedHeight) : 0}%` }} />
                </div>
  ...
  const rows = metric === "gross"
    ? [
        { label: "La Peskerezh", value: 10480, width: 42, tone: "blue" as Tone },
        { label: "Apt 4 · Balcon", value: 6480, width: 26, tone: "blue" as Tone },
        { label: "Apt 5 · Sous les toits", value: 4320, width: 17, tone: "mustard" as Tone },
        { label: "Apt 2 · Jardin", value: 3580, width: 15, tone: "orange" as Tone },
      ]
    : [
        { label: "Ménage", value: 3240, width: 56, tone: "mustard" as Tone },
        { label: "Commissions", value: 1120, width: 22, tone: "blue" as Tone },
        { label: "Interventions", value: 430, width: 12, tone: "orange" as Tone },
        { label: "Linge / autres", value: 650, width: 16, tone: "navy" as Tone },
      ];

  return (
    <div className="rounded-[1.55rem] bg-white/8 p-4 ring-1 ring-white/12">
      <p className="text-sm font-black text-white">{metric === "gross" ? "CA brut par logement" : "Coûts variables suivis"}</p>
      <p className="mt-1 text-xs font-bold text-white/50">Lecture simple pour décider sans se perdre dans les détails.</p>
      <div className="mt-4 space-y-3">
        {rows.map((row) => (
          <div key={row.label} className="grid grid-cols-[112px_1fr_82px] items-center gap-2 text-xs font-black sm:grid-cols-[150px_1fr_96px]">
            <span className="truncate text-white/62">{row.label}</span>
