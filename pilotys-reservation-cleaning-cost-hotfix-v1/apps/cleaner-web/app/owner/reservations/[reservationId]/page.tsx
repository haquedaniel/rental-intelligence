...
import ReservationInstructionSubmitButton from "@/components/owner/ReservationInstructionSubmitButton";
...
function directionClass(value?: string | null): string {
  switch (value) {
    ...
    case "system":
      return "bg-[#112532]/6 text-[#112532]/48 ring-[#112532]/10";
    default:
      return "bg-white text-[#112532]/76 ring-[#112532]/10";
  }
}
...
export default async function OwnerReservationPage({
  ...
  const preparationInstructionLocked = [preparationMission, checkoutMission, ...cleaningRequests].some((request) =>
  ...
  // The operational panel intentionally includes both:
  // - the mission that prepared this reservation; and
  // - the checkout mission generated by this reservation.
  //
  // Only the checkout mission is economically attributable to this stay.
  // Deduplicate because it may be present both in `checkoutMission` and in
  // `cleaningRequests`.
  const attributableCleaningRequests = Array.from(
    new Map(
      [checkoutMission, ...cleaningRequests]
        .filter((request): request is Row => Boolean(request))
        .filter(
          (request) =>
            !["cancelled", "refused"].includes(String(request.status ?? "")),
        )
        .filter(
          (request) =>
            (context?.checkout_mission_id &&
              String(request.id) === String(context.checkout_mission_id)) ||
  ...
  const cleanerCost = sumRows(attributableCleaningRequests, [
    "total_cost_eur",
    "cleaning_cost_eur",
    "amount_eur",
  ]);
    ...
    <main className="min-h-screen bg-[#F6F3EF] pb-28 text-[#112532]">
      ...
      <div className="mx-auto max-w-7xl space-y-5 px-4 py-5 sm:px-6 lg:px-8">
        ...
        <section className="rounded-[2rem] bg-[#FFF5DD] p-5 text-[#8A4D00] shadow-sm ring-1 ring-[#F4B044]/25">
          ...
          <h2 className="mt-2 text-2xl font-black">Add any important notes</h2>
          ...
          <form action={saveReservationPreparationNote} className="mt-4">
            ...
            />
            <ReservationInstructionSubmitButton locked={preparationInstructionLocked} />
          </form>
