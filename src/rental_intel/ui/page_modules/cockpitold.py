...
def _build_listing_meta(reservations: pd.DataFrame) -> pd.DataFrame:
    """Small display table for the operating map.

    The component works without this, but passing a friendly label keeps the
    calendar readable and supports owners with one or many listings.
    """
    if reservations is None or reservations.empty or "listing_id" not in reservations.columns:
        return pd.DataFrame(columns=["listing_id", "listing_name", "subtitle"])
...
def _cleaning_events_from_due(
    ...
    the map will still derive 'ménage à confirmer' from departures.
    """
    ...
    if not date_col or not listing_col:
        return pd.DataFrame(columns=["listing_id", "date", "status", "label"])
    ...
    return pd.DataFrame(rows, columns=["listing_id", "date", "status", "label"])
