...
def publish(property_id: str | None=None, limit: int=100, dry_run: bool=False) -> int:
    db=get_supabase_client(); q=db.table("pricing_publication_actions").select("*").eq("status","proposed").eq("mode","apply").order("date").limit(limit)
    ...
    for action in actions:
        try:
            ...
            db.table("pricing_publication_actions").update({"status":"applying"}).eq("id",action["id"]).execute()
            ...
            db.table("pricing_publication_actions").update({"status":"applied","payload":payload,"response":{"write":response,"offer_after":after},"applied_at":datetime.now(timezone.utc).isoformat(),"error":None}).eq("id",action["id"]).execute()
            ...
            db.table("pricing_publication_actions").update({"status":"validation_failed","error":str(exc)}).eq("id",action["id"]).execute()
