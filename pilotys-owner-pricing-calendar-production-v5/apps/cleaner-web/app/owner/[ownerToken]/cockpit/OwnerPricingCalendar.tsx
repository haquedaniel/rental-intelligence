...
export function OwnerPricingCalendar({
  ...
  const available = useMemo(
    () =>
      data.listings.filter((listing) =>
        data.pricingCalendar.some((row) => row.listingId === listing.id),
      ),
    [data],
  );
  const preferred =
    available.find((listing) => selectedListingIds.includes(listing.id)) ||
    available[0];
  const [manualListingId, setManualListingId] = useState<string | null>(null);
  const listing =
    available.find((item) => item.id === manualListingId) || preferred;
  const listingRows = useMemo(
    () => data.pricingCalendar.filter((row) => row.listingId === listing?.id),
    [data.pricingCalendar, listing?.id],
  );
  const months = useMemo(
    () => [...new Set(listingRows.map((row) => row.date.slice(0, 7)))].sort(),
    [listingRows],
  );
  const currentMonthIndex = Math.max(
    0,
    months.findIndex((month) => month >= data.today.slice(0, 7)),
  );
    ...
    return (
      <section className="rounded-[1.7rem] bg-white p-5 shadow-[0_10px_30px_rgba(17,37,50,0.06)] ring-1 ring-[#112532]/8">
        ...
        <p className="mt-3 text-sm font-bold text-[#112532]/55">
          Aucun calendrier calculé pour les logements sélectionnés.
        </p>
      </section>
    );
  }
  ...
  const weeks = Array.from({ length: count / 7 }, (_, index) =>
    cells.slice(index * 7, index * 7 + 7),
  );

  const marketValues = monthRows
    .map((row) => Math.abs(Number(row.marketSignalPct || 0)))
    .filter((value) => value > 0.01)
    .sort((a, b) => a - b);
  const p90 = marketValues.length
    ? marketValues[
        Math.min(marketValues.length - 1, Math.floor(marketValues.length * 0.9))
      ]
    : 0;
  ...
  const seasons = data.pricingSeasons.filter(
    (season) =>
      season.listingId === listing.id &&
      season.startDate <= last &&
      season.endDate >= first,
  );
  const seasonById = new Map(seasons.map((season) => [season.id, season]));
  const reservations = data.pricingReservations.filter(
    (reservation) =>
    ...
    <section className="rounded-[1.7rem] bg-white p-4 shadow-[0_10px_30px_rgba(17,37,50,0.06)] ring-1 ring-[#112532]/8 sm:p-5">
      ...
      {available.length > 1 && (
        <select
          ...
          className="mt-4 w-full rounded-2xl border border-[#112532]/10 bg-white px-4 py-3 text-sm font-black text-[#112532]"
        >
          {available.map((item) => (
      ...
      <div className="mt-4 overflow-hidden rounded-[1.45rem] border border-[#112532]/10">
        <div className="grid grid-cols-[42px_1fr_42px] items-center gap-2 px-2 py-2">
          <button
            ...
            onClick={() => setMonthOffset(Math.max(0, safeIndex - 1))}
            className="h-10 rounded-xl border border-[#112532]/12 disabled:opacity-25"
          >
            ←
          </button>
          <h3 className="text-center text-lg font-black capitalize">
          ...
          <button
            ...
            onClick={() =>
              setMonthOffset(Math.min(months.length - 1, safeIndex + 1))
            }
            className="h-10 rounded-xl border border-[#112532]/12 disabled:opacity-25"
          >
            →
        ...
        <div className="grid grid-cols-7 border-t border-[#112532]/8">
          {["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((day) => (
            <div
              className="py-1.5 text-center text-[10px] font-black text-[#112532]/60"
              key={day}
            >
              {day}
            </div>
        ...
        <div>
          {weeks.map((week, weekIndex) => {
            ...
            const segments = reservations.flatMap((reservation) => {
              ...
              const startPosition =
                start === reservation.start ? startIndex + 0.5 : 0;
              const endPosition = end === reservation.end ? endIndex + 0.5 : 7;
              return [
                {
                  ...reservation,
                  left: (startPosition / 7) * 100,
                  right: 100 - (endPosition / 7) * 100,
                },
              ];
            });
            return (
                    ...
                    <button
                      ...
                      onClick={() =>
                        ...
                        window.dispatchEvent(
                          new CustomEvent("pilotys-price-detail", {
                            detail: row,
                          }),
                        )
                      }
                        ...
                        {row?.sourceSeasonId &&
                          seasonById.has(row.sourceSeasonId) && (
                            <i
                              ...
                              title={seasonById.get(row.sourceSeasonId)?.name}
                              style={{
                                backgroundColor:
                                  SEASON_COLORS[
                                    hash(row.sourceSeasonId) %
                                      SEASON_COLORS.length
                                  ],
                              }}
                            />
                          )}
                ...
                <div className="pointer-events-none absolute inset-x-0 top-[23px] z-10 h-[16px]">
                  {segments.map((segment, index) => (
                    <div
                      ...
                      title={`${segment.guest} · ${money(segment.total)}`}
                    >
        ...
        <div className="space-y-2 border-t border-[#112532]/8 px-3 py-3 text-[10px]">
          {seasons.some((season) =>
            monthRows.some((row) => row.sourceSeasonId === season.id),
          ) && (
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
              <strong className="w-14">Saisons</strong>
              {seasons
                .filter((season) =>
                  monthRows.some((row) => row.sourceSeasonId === season.id),
                )
                .map((season) => (
                  <span className="flex items-center gap-2" key={season.id}>
                    <i
                      className="h-2 w-2 rounded-full"
                      style={{
                        backgroundColor:
                          SEASON_COLORS[hash(season.id) % SEASON_COLORS.length],
                      }}
                    />
                    {season.name}
                  </span>
          ...
          <div className="flex items-center gap-2">
            <strong className="w-14 shrink-0">Marché</strong>
          ...
          <div className="ml-16 flex justify-between text-[9px] font-bold text-[#112532]/45">
      ...
      {missingCount > 0 && (
        <details className="mt-3 rounded-2xl border border-[#112532]/8 px-4 py-3 text-sm">
          <summary className="cursor-pointer font-black">
            {missingCount} logement(s) sans calendrier calculé
          </summary>
          <p className="mt-2 text-[#112532]/55">
            Ils apparaîtront ici après leur premier calcul tarifaire.
          </p>
        </details>
      )}
...
function PriceDetailDrawer() {
  ...
  return (
    <div
      className="fixed inset-0 z-[70] flex items-end bg-[#071821]/35"
      onClick={() => setRow(null)}
    >
      <div
        className="mx-auto w-full max-w-md rounded-t-[1.8rem] bg-white p-5 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <button onClick={() => setRow(null)} className="float-right text-2xl">
          ×
        </button>
        <p className="text-xs font-black uppercase tracking-[0.18em] text-[#E0680E]">
          Pourquoi ce prix ?
        ...
        <h3 className="mt-1 text-xl font-black capitalize">
          {noon(row.date).toLocaleDateString("fr-FR", {
            weekday: "long",
            day: "numeric",
            month: "long",
          })}
        ...
        <div className="mt-5 space-y-3">
          {(row.explanationSteps || []).map((step: any, index: number) => (
            <div
              className="flex items-start justify-between gap-4 border-t border-[#112532]/8 pt-3"
              key={index}
            >
              <div>
                <strong>{step.label}</strong>
                <p className="mt-1 text-xs text-[#112532]/55">
                  {step.explanation}
                </p>
              </div>
              <strong className="shrink-0">{money(step.after_eur)}</strong>
            </div>
          ))}
        </div>
      </div>
...
export function OwnerJournalHeadlines({ data }: { data: OwnerCockpitData }) {
  return (
    <section className="overflow-hidden rounded-[1.7rem] bg-white shadow-[0_10px_30px_rgba(17,37,50,0.06)] ring-1 ring-[#112532]/8">
      <div className="flex items-end justify-between p-5 pb-3">
        ...
        <a
          href={`/owner/${data.owner.token}/activity`}
          className="text-sm font-black text-[#E0680E]"
        >
          Voir tout
        </a>
      </div>
      {data.journalHeadlines.length ? (
        data.journalHeadlines.slice(0, 8).map((item) => (
          <a
            href={`/owner/${data.owner.token}/activity`}
            key={item.id}
            className="grid grid-cols-[1fr_auto] gap-3 border-t border-[#112532]/8 px-5 py-3 hover:bg-[#F8FAFB]"
          >
            <div>
            ...
            <time className="text-right text-[10px] font-bold text-[#112532]/45">
        ...
        <p className="border-t border-[#112532]/8 p-5 text-sm font-bold text-[#112532]/50">
