...
function requestIsIntervention(request: Row) {
  ...
  return ["garden_lawn", "maintenance_check", "inventory_check"].includes(
    serviceType,
  );
}
...
function dailyRowsForProperty(dailyRows: Row[], propertyId: string) {
  return dailyRows.filter(
    (row) => !row.property_id || String(row.property_id) === propertyId,
  );
}
...
function requestTimelineDateKey(request: Row): string | null {
  ...
  if (request.scheduled_start_at)
    return parisDateKey(request.scheduled_start_at);
...
function hostPayoutAllocated(row: Row) {
  return numberValue(row, [
    "host_payout_allocated",
    "host_payout_allocated_eur",
  ]);
}
...
function monthlyRowsForMonth(rows: Row[], yearMonth: string) {
  return preferGranularRows(
    rows.filter((row) => String(row.year_month ?? "") === yearMonth),
  );
}

function dailyRowsForMonth(rows: Row[], yearMonth: string) {
  return preferGranularRows(
    rows.filter((row) => String(row.date ?? "").slice(0, 7) === yearMonth),
  );
}
...
function expenseSource(row: Row): string {
  return String(
    row.expense_source ?? row.source ?? row.cost_source ?? row.kind ?? "",
  );
}
...
function addBreakdown(
  map: Map<string, ExpenseBreakdownItem>,
  label: string,
  amount: number,
  count = 1,
) {
...
function exactVariableExpenseAmount({
  ...
  const variableRows = expenseRows.filter(
    (row) => expenseSource(row) === "variable_period_costs",
  );
  const directlyCountedRows = expenseRows.filter(
    (row) => expenseSource(row) !== "variable_period_costs",
  );
  ...
  for (const daily of periodDaily) {
    ...
    const matches = variableRows.filter(
      (expense) =>
        ...
        String(expense.year_month ?? "") ===
          String(daily.year_month ?? String(daily.date ?? "").slice(0, 7)),
    );

    for (const expense of matches) {
      addBreakdown(
        breakdown,
        labelForExpense(expense),
        expenseAmountPerDay(expense),
      );
    }
  }

  const items = Array.from(breakdown.values()).sort(
...
function buildListings({
  ...
  return properties.map((property, index) => {
    ...
    let revenue = propertyMonthlyRows.reduce(
      (total, row) => total + hostPayout(row),
      0,
    );
    ...
    if (revenue === 0) {
      revenue = yearReservations
        .filter(
          (reservation) =>
            String(reservation.property_id ?? "") === String(property.id),
        )
        .reduce(
          (total, reservation) =>
            total +
            reservationAmount({ reservation, dailyRows, monthlyRows, targets }),
          0,
        );
    }

    const booked = propertyMonthlyRows.reduce(
      (total, row) => total + bookedNights(row),
      0,
    );
    const occupancyPct = propertyMonthlyRows.reduce(
      (max, row) => Math.max(max, numberValue(row, ["occupancy_pct"])),
      0,
    );
    ...
    return {
      ...
      short: String(property.name || index + 1)
        .slice(0, 1)
        .toUpperCase(),
      ...
      revenue,
      occupancy:
        occupancyPct || Math.min(100, Math.round((booked / 365) * 100)),
    };
  });
}
...
function buildMonthlyRevenue({
  ...
  return Array.from({ length: 12 }, (_, monthIndex) => {
    ...
    const totalOnBooks = rows.reduce(
      (total, row) => total + hostPayout(row),
      0,
    );
...
function dailyAllocatedRevenueForDate(rows: Row[], dateKey: string) {
  return preferGranularRows(
    rows.filter((row) => String(row.date ?? "") === dateKey),
...
function buildFinancial({
  ...
  const grossAnnualRevenue = monthly.reduce(
    (total, row) => total + row.realised + row.future,
    0,
  );

  const yearDaily = preferGranularRows(
    dailyRows.filter((row) => rowDateInRange(row, yearStart, yearEnd)),
  );
  const bookingExpenseRows = expenses.filter(
    (row) =>
      row.expense_source === "booking_expenses" &&
      expenseDateInRange(row, yearStart, yearEnd),
  );
  const variableRowsForSelectedMonths = expenses.filter(
    (row) =>
      row.expense_source === "variable_period_costs" &&
      monthInRange(row, yearStart, yearEnd),
  );
...
function cleaningCoverageForReservation(
  reservation: Row,
  requestsByReservationId: Map<string, Row[]>,
) {
  const linkedRequests = (
    requestsByReservationId.get(String(reservation.id)) ?? []
  ).filter(requestVisibleInPlanning);

  if (linkedRequests.length === 0) {
    return {
      cleaningState: "none" as const,
      cleaningRequest: null as Row | null,
    };
  }

  const accepted =
    linkedRequests.find((request) =>
      ["accepted", "completed", "report_submitted"].includes(
...
function buildPlanningReservations({
  ...
  return reservations
    ...
    .map((reservation) => {
      ...
      const displayEnd =
        checkout > addDays(planningEnd, 1) ? addDays(planningEnd, 1) : checkout;
      const span = Math.max(1, daysBetween(displayStart, displayEnd));
      const price = reservationAmount({
        reservation,
        dailyRows,
        monthlyRows,
        targets,
      ...
      const nights = Math.max(1, daysBetween(checkin, checkout));
      const coverage = cleaningCoverageForReservation(
        reservation,
        requestsByReservationId,
      );
      const missionDate = coverage.cleaningRequest
        ? requestTimelineDateKey(coverage.cleaningRequest)
        : null;
      const cleaningDay = missionDate
        ? Math.max(
            1,
            Math.min(maxDay, daysBetween(planningStart, missionDate) + 1),
          )
        : null;
...
function timelineTitleForRequest(request: Row) {
  ...
  if (["completed", "report_submitted", "problem_reported"].includes(status)) {
    return status === "problem_reported"
      ? `${label} · problème`
      : `${label} terminé`;
  }
...
function timelineDetailForRequest(
  request: Row,
  cleanersById: Record<string, Row>,
) {
  const status = String(request.status ?? "");
  ...
  if (["completed", "report_submitted"].includes(status))
    return "Photos et rapport disponibles";
...
function buildOpportunities({
  ...
  const gapOpportunities: Opportunity[] = listings
    .slice(0, 2)
    .map((listing, index) => {
...
async function maybeSignedCleanerImages(
  supabase: ReturnType<typeof getSupabaseAdmin>,
  rows: Row[],
): Promise<Row[]> {
  return Promise.all(
    rows.map(async (row) => ({
      ...row,
      profile_photo_signed_url: await signedUrl(
        supabase,
...
async function buildPropertyCoverMap(
  supabase: ReturnType<typeof getSupabaseAdmin>,
  photos: Row[],
) {
  ...
  for (const photo of photos) {
    ...
    coverByPropertyId[propertyId] = await signedUrl(
      supabase,
...
export async function getOwnerCockpitData(
  ownerTokenParam: string,
): Promise<OwnerCockpitData> {
  ...
  if (ownerError) {
    throw new Error(
      `Impossible de charger le propriétaire : ${ownerError.message}`,
    );
  }
  ...
  if (propertiesError) {
    throw new Error(
      `Impossible de charger les logements : ${propertiesError.message}`,
    );
  }
  ...
  if (propertyIds.length === 0) {
    return {
      ...
      monthlyRevenue: MONTH_LABELS.map((month) => ({
        month,
        realised: 0,
        future: 0,
        target: 0,
  ...
  const [
    ...
    supabase
      ...
      .select(
        "property_id,date,final_price,market_signal_pct,source_season_id,explanation_steps",
      )
      .in("property_id", propertyIds)
      .gte("date", planningStart)
      .lte("date", planningEnd)
  ...
  for (const [label, result] of [
    ...
    if ("error" in result && result.error) {
      throw new Error(
        `Impossible de charger ${label} : ${result.error.message}`,
      );
    }
  }
  ...
  const signedCleaners = await maybeSignedCleanerImages(
    supabase,
    cleaners as Row[],
  );
  const cleanersById = Object.fromEntries(
    signedCleaners.map((cleaner) => [cleaner.id, cleaner]),
  );

  const coverByPropertyId = await buildPropertyCoverMap(
    supabase,
    (propertyPhotosResult.data ?? []) as Row[],
  );
  ...
  const analyticsDaily = scopedRows(
    analyticsDailyResult.data ?? [],
    propertyIds.map(String),
  );
  const analyticsMonthly = scopedRows(
    analyticsMonthlyResult.data ?? [],
    propertyIds.map(String),
  );
  const targets = scopedRows(
    analyticsTargetsResult.data ?? [],
    propertyIds.map(String),
  );
  const expenses = scopedRows(
    analyticsExpensesResult.data ?? [],
    propertyIds.map(String),
  );
  ...
  const activeDailyRevenueFromReservations = activeReservations.reduce(
    (sum, reservation) => sum + reservationDailyRevenue(reservation),
    0,
  );
  ...
  const financial = {
    ...financialBase,
    realisedAtStartOfToday: Math.max(
      0,
      financialBase.realisedRevenue - activeDailyRevenue,
    ),
    activeDailyRevenue,
  };
  ...
  const planningMarkers = buildMarkers({
    requests,
    cleanersById,
    planningStart,
    planningEnd,
  ...
  const pricingCalendar = (pricingCalendarResult.data ?? []).map(
    (row: Row) => ({
      ...
      sourceSeasonId: row.source_season_id
        ? String(row.source_season_id)
        : null,
      explanationSteps: Array.isArray(row.explanation_steps)
  ...
  const pricingSeasons = (pricingSeasonsResult.data ?? []).map((row: Row) => ({
    id: String(row.id),
    listingId: String(row.property_id),
    name: String(row.name || "Saison"),
    startDate: String(row.start_date),
    endDate: String(row.end_date),
  }));
  const pricingReservations = reservations
    .map((row: Row) => ({
      id: String(row.id),
      listingId: String(row.property_id),
      guest: String(
        row.guest_name || row.guest_full_name || row.name || "Réservation",
      ),
      start: parisDateKey(row.checkin_at),
      end: parisDateKey(row.checkout_at),
  ...
  const journalHeadlines = (journalSituationsResult.data ?? []).map(
    (row: Row) => ({
      id: String(row.id),
      headline: String(row.headline || "Mise à jour Pilotys"),
