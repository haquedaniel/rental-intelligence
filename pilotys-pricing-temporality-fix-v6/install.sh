...
STAMP="$(date +%Y%m%d-%H%M%S)"; BACKUP="$ROOT/.pilotys-backups/pricing-temporality-v6-$STAMP"; mkdir -p "$BACKUP"
...
for f in CurvePresetSelector.tsx SeasonEditor.tsx PricingDashboard.tsx; do copy "$PKG/frontend/components/pricing/$f" "$ROOT/apps/cleaner-web/components/pricing/$f"; done
python -m py_compile "$ROOT/src/rental_intel/pricing/engine.py"
echo "Installed. Backup: $BACKUP"
echo "Next: docker compose up -d --build cockpit cleaner-web pricing-api"
