...
def timestamp_value(row: dict[str, Any], candidates: list[str]) -> str | None:
    value = field(row, candidates)
    if not value:
        return None
    return value
...
def payload_from_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_property_id": field(row, ["source_property_id"]),
        "source_room_id": field(row, ["source_room_id"]),
        "api_source": field(row, ["api_source"]),
        "api_reference": field(row, ["api_reference"]),
        "referer": field(row, ["referer"]),
        ...
        "customer_paid_eur": number(row, ["customer_paid_eur", "price_total", "price_total_eur", "gross_booking_value"]),
        "tourist_tax_eur": number(row, ["tourist_tax_eur", "tourist_tax"]),
        "channel_commission_eur": number(row, ["channel_commission_eur", "channel_commission", "commission"]),
        "price_total_eur": number(row, ["price_total_eur", "price_total"]),
        "invoice_total_eur": number(row, ["invoice_total_eur", "invoice_total"]),
        "adr_host_payout_eur": number(row, ["adr_host_payout_eur", "adr_host_payout"]),

        "booking_time": timestamp_value(row, ["booking_time"]),
        "modified_time": timestamp_value(row, ["modified_time"]),
        "cancel_time": timestamp_value(row, ["cancel_time"]),
        "synced_at": timestamp_value(row, ["synced_at"]) or __import__("datetime").datetime.now(__import__("datetime").timezone.utc).isoformat(),
