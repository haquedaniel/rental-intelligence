...
export default async function PricingPage({ searchParams }: { searchParams: Promise<{ property?: string }> }) {
  ...
  const safeSeasons = (seasons ?? []) as Row[];
  const safeCalendar = (calendar ?? []) as Row[];
  const safeActions = (actions ?? []) as Row[];
  ...
  return <main style={{maxWidth:1280,margin:"0 auto",padding:"28px 20px",fontFamily:"system-ui"}}>
    ...
    {propertyId && <>
      ...
      <section style={{border:"1px solid #e2e8f0",borderRadius:16,padding:20,marginBottom:20}}><h2>Saisons et pics connus</h2>
        ...
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr><th>Saison</th><th>Période</th><th>Semaine</th><th>Week-end</th><th>Min.</th><th></th></tr></thead><tbody>{safeSeasons.map((x:Row)=><tr key={x.id}><td>{x.name}</td><td>{x.start_date} → {x.end_date}</td><td>{eur(x.weekday_price)}</td><td>{eur(x.weekend_price)}</td><td>{x.min_stay} nuits</td><td><form action={deleteSeason}><input type="hidden" name="id" value={x.id}/><button>Retirer</button></form></td></tr>)}</tbody></table></div>
      </section>

      <section style={{border:"1px solid #e2e8f0",borderRadius:16,padding:20,marginBottom:20}}><h2>Calendrier calculé · 90 jours</h2><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr><th>Date</th><th>État</th><th>Plan</th><th>Ajustement</th><th>Prix final</th><th>Min.</th><th>Stratégie</th><th>Publication</th></tr></thead><tbody>{safeCalendar.map((x:Row)=><tr key={x.date} style={{opacity:x.occupied?.6:1}}><td>{x.date}</td><td>{x.occupied?"Occupé":"Libre"}</td><td>{eur(x.base_price)}</td><td>{eur(x.strategy_adjustment)}</td><td><strong>{eur(x.final_price)}</strong></td><td>{x.min_stay}</td><td>{x.strategy}</td><td>{x.publication_status}</td></tr>)}</tbody></table></div></section>

      <section style={{border:"1px solid #e2e8f0",borderRadius:16,padding:20}}><h2>Dernières décisions</h2>{safeActions.length===0?<p>Aucune décision générée.</p>:<div style={{overflowX:"auto"}}><table style={{width:"100%"}}><thead><tr><th>Date</th><th>Ancien</th><th>Cible</th><th>Min.</th><th>Motif</th><th>État</th></tr></thead><tbody>{safeActions.map((x:Row)=><tr key={x.id}><td>{x.date}</td><td>{eur(x.old_price)}</td><td>{eur(x.target_price)}</td><td>{x.target_min_stay}</td><td>{x.reason}</td><td>{x.status}</td></tr>)}</tbody></table></div>}</section>
    </>}
