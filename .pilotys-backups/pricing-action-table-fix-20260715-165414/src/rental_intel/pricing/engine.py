...
def regenerate(property_id: str | None = None) -> int:
    ...
    for raw in settings:
        ...
        if actions:
            ...
            db.table("pricing_actions").update({"status": "superseded", "updated_at": now.isoformat()}).eq("property_id", setting.property_id).eq("status", "proposed").execute()
            db.table("pricing_actions").insert(actions).execute()
        total += len(new_rows)
    return total
